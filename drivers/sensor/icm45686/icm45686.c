...
static int icm45686_set_accel_fs(const struct device *dev, uint16_t fs)
{
	...
	return icm45686_update_register(&cfg->bus, REG_ACCEL_CONFIG0,
...
static int icm45686_set_gyro_fs(const struct device *dev, uint16_t fs)
{
        ...
        return icm45686_update_register(&cfg->bus, REG_GYRO_CONFIG0,
...
static int icm45686_set_gyro_odr(const struct device *dev, uint16_t rate)
{
        ...
        return icm45686_update_register(&cfg->bus, REG_GYRO_CONFIG0, (uint8_t)MASK_GYRO_ODR,
                                            temp);
}
...
int icm45686_turn_on_sensor(const struct device *dev)
{
	...
	value = FIELD_PREP(MASK_ACCEL_MODE, BIT_ACCEL_MODE_LNM) | FIELD_PREP(MASK_GYRO_MODE, BIT_GYRO_MODE_LNM);
...
#define ICM45686_INIT(inst)                                                                        \
	...
	SENSOR_DEVICE_DT_INST_DEFINE(inst, icm45686_init, NULL, &icm45686_driver_##inst,           \
			      &icm45686_cfg_##inst, POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY,      \
			      &icm45686_driver_api);

...
/*
 * Bus-specific functionality for ICM456XX accessed via SPI.
 */
...
static int icm456xx_bus_check_spi(const union icm456xx_bus *bus)
{
	return spi_is_ready_dt(&bus->spi) ? 0 : -ENODEV;
}

static int icm456xx_reg_read_spi(const union icm456xx_bus *bus, uint8_t start, uint8_t *buf,
				 uint32_t size)
...
static int icm456xx_reg_write_spi(const union icm456xx_bus *bus, uint8_t reg, uint8_t *buf,
				  uint32_t size)
{
...
const struct icm456xx_bus_io icm456xx_bus_io_spi = {
	.check = icm456xx_bus_check_spi,

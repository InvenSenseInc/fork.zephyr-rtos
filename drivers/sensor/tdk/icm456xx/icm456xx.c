...
static inline int icm456xx_reg_read(const struct device *dev, uint8_t reg, uint8_t *buf,
		uint32_t size)
{
	return icm456xx_bus_read(dev, reg, buf, size);
}
...
static inline int icm456xx_reg_write(const struct device *dev, uint8_t reg, const uint8_t *buf,
		uint32_t size)
{
	return icm456xx_bus_write(dev, reg, buf, size);
}
...
int icm456xx_sample_fetch_gyro(const struct device *dev)
{
	struct icm456xx_data *data = dev->data;
	uint8_t buffer[ACCEL_DATA_SIZE];
...
static int icm456xx_fetch_from_registers(const struct device *dev, enum sensor_channel chan)
{
	...
	if (int_state.INV_UI_DRDY) {
		...
		case SENSOR_CHAN_ALL:
			...
			err |= icm456xx_sample_fetch_gyro(dev);
...
#if CONFIG_SPI_RTIO
	if ((data->rtio.type == ICM456XX_BUS_SPI) && !spi_is_ready_iodev(data->rtio.iodev)) {
		LOG_ERR("Bus is not ready");
		return -ENODEV;
	}
#endif
#if CONFIG_I2C_RTIO
	if ((data->rtio.type == ICM456XX_BUS_I2C) && !i2c_is_ready_iodev(data->rtio.iodev)) {
		LOG_ERR("Bus is not ready");
		return -ENODEV;
	}
#endif
...
static void icm456xx_complete_result(struct rtio *ctx,
...
void icm456xx_submit_one_shot(const struct device *dev,
                                            struct rtio_iodev_sqe *iodev_sqe)
{
        const struct sensor_read_config *cfg = iodev_sqe->sqe.iodev->data;
        const struct sensor_chan_spec *const channels = cfg->channels;
        const size_t num_channels = cfg->count;
        uint32_t min_buf_len = sizeof(struct icm456xx_encoded_data);
        int err;
        uint8_t *buf;
        uint32_t buf_len;
        struct icm456xx_encoded_data *edata;
        struct icm456xx_data *data = dev->data;
        ...
        edata = (struct icm456xx_encoded_data *)buf;

        err = icm456xx_encode(dev, channels, num_channels, buf);
        if (err != 0) {
...
static void icm456xx_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
        const struct sensor_read_config *cfg = iodev_sqe->sqe.iodev->data;

        if (!cfg->is_streaming) {
                icm456xx_submit_one_shot(dev, iodev_sqe);
        } else if (IS_ENABLED(CONFIG_ICM456XX_STREAM)) {
                icm456xx_stream_submit(dev, iodev_sqe);
        } else {
...
#ifdef CONFIG_SENSOR_ASYNC_API
	.get_decoder = icm456xx_get_decoder,
        .submit = icm456xx_submit,
#endif

};

#define ICM456XX_SPI_CFG SPI_OP_MODE_MASTER | SPI_MODE_CPOL | SPI_MODE_CPHA | SPI_WORD_SET(8) | SPI_TRANSFER_MSB

#define ICM456XX_INIT(inst)                                                                            \
	RTIO_DEFINE(icm456xx_rtio_ctx_##inst, 8, 8);                                                    \
                                                                                                       \
	COND_CODE_1(DT_INST_ON_BUS(inst, i3c),                                                     \
                    (I3C_DT_IODEV_DEFINE(icm456xx_bus_##inst,                                      \
                                         DT_DRV_INST(inst))),                                      \
        (COND_CODE_1(DT_INST_ON_BUS(inst, i2c),                                                    \
                    (I2C_DT_IODEV_DEFINE(icm456xx_bus_##inst,                                      \
                                         DT_DRV_INST(inst))),                                      \
                    ())));                                                                         \
        COND_CODE_1(DT_INST_ON_BUS(inst, spi),                                                     \
                    (SPI_DT_IODEV_DEFINE(icm456xx_bus_##inst,                                      \
                                         DT_DRV_INST(inst),                                        \
                                         SPI_OP_MODE_MASTER | SPI_WORD_SET(8) | SPI_TRANSFER_MSB,  \
                                         0U)),                                                     \
                    ());									\
	/* Define the driver's private data structure */                                               \
	static struct icm456xx_data icm456xx_data_##inst = {                                            \
		.rtio = {                                                                              \
			.iodev = &icm456xx_bus_##inst,							\
			.ctx = &icm456xx_rtio_ctx_##inst,                                               \
			.type = ICM456XX_BUS_SPI,                                                      \
		},                                                                                     \
	};                                                                                             \
                                                                                                       \
	/* Define the driver's configuration structure, using the name from your code */               \
	static const struct icm456xx_config icm456xx_cfg_##inst = {                                     \
		...
		.gyro_filt_bw = DT_INST_ENUM_IDX(inst, gyro_filt_bw_hz),                                \
                                                                                                       ...
                                                                                                       \
	/* Use the standard Zephyr macro to create the device, referencing the config struct */        \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, icm456xx_init,                                              \
				   NULL,                                                               \
				   &icm456xx_data_##inst,                                              \
				   &icm456xx_cfg_##inst,                                               \
				   POST_KERNEL,                                                        \
...
/* This block correctly invokes the initialization for each enabled instance */
#define DT_DRV_COMPAT invensense_icm45686
#if DT_HAS_COMPAT_STATUS_OKAY(DT_DRV_COMPAT)
DT_INST_FOREACH_STATUS_OKAY(ICM456XX_INIT)
#endif
#undef DT_DRV_COMPAT

...
#if DT_HAS_COMPAT_ON_BUS_STATUS_OKAY(invensense_icm456xx, i3c)
...
LOG_MODULE_REGISTER(ICM456XX_STREAM, CONFIG_SENSOR_LOG_LEVEL);
...
static void icm456xx_handle_event_actions(struct rtio *ctx,
					  const struct rtio_sqe *sqe,
					  void *arg)
{
	const struct device *dev = (const struct device *)arg;
	...
	struct icm456xx_encoded_data *buf;
	uint32_t buf_len;
	uint32_t buf_len_required = sizeof(struct icm456xx_encoded_header);
	...
	if (should_read_fifo(read_cfg, int_status)) {
		buf_len_required += (data->stream.data.fifo_count *
				     sizeof(struct icm456xx_encoded_fifo_payload));
	} else if (should_read_data(read_cfg, int_status)) {
		buf_len_required += sizeof(struct icm456xx_encoded_payload);
	}
	...
	if (should_read_fifo(read_cfg, int_status)) {
		 ...
		 */
		buf->header.accel_fs = ICM456xx_DT_ACCEL_FS_32;
		buf->header.gyro_fs = ICM456xx_DT_GYRO_FS_4000;
...
static void icm456xx_event_handler(const struct device *dev)
{
	struct icm456xx_data *data = dev->data;
	uint8_t val = 0;
	uint64_t cycles;
...
#if DT_HAS_COMPAT_ON_BUS_STATUS_OKAY(invensense_icm456xx, i3c)
static int icm456xx_ibi_cb(struct i3c_device_desc *target,
			   struct i3c_ibi_payload *payload)
{
	icm456xx_event_handler(target->dev);

	return 0;
}
#endif

static void icm456xx_gpio_callback(const struct device *gpio_dev,
				   struct gpio_callback *cb,
				   uint32_t pins)
{
	struct icm456xx_stream *stream = CONTAINER_OF(cb,
						      struct icm456xx_stream,
						      cb);
	const struct device *dev = stream->dev;

	icm456xx_event_handler(dev);
}

static inline bool settings_changed(const struct icm456xx_stream *a,
				    const struct icm456xx_stream *b)
{
			    ...
			    struct rtio_iodev_sqe *iodev_sqe)
{
	 ...
	 * in such case.
	 */
	struct icm456xx_stream stream = {0};
	...
	if (settings_changed(&data->stream, &stream)) {
		...
		err = icm456xx_bus_write(dev, REG_INT1_CONFIG0, &val, 1);
		if (err) {
		...
		err = icm456xx_bus_read(dev, REG_INT1_STATUS0, &val, 1);
		if (err) {
		...
		err = icm456xx_bus_write(dev, REG_FIFO_CONFIG3, &val, 1);
		if (err) {
		...
		err = icm456xx_bus_write(dev, REG_FIFO_CONFIG0, &val, 1);
		if (err) {
		...
		if (data->stream.settings.enabled.fifo_ths ||
			...
			val = REG_FIFO_CONFIG2_FIFO_WM_GT_THS(true) |
			      REG_FIFO_CONFIG2_FIFO_FLUSH(true);
			err = icm456xx_bus_write(dev, REG_FIFO_CONFIG2, &val, 1);
			if (err) {
			...
			err = icm456xx_bus_write(dev, REG_FIFO_CONFIG1_0, (uint8_t *)&fifo_ths, 2);
			if (err) {
...
int icm456xx_stream_init(const struct device *dev)
{
	...
	if (cfg->int_gpio.port) {
		...
		err = icm456xx_bus_write(dev, REG_INT1_CONFIG2, &val, 1);
		if (err) {
	...
	/** I3C devices use IBI only if no GPIO INT pin is defined. */
		...
		data->rtio.i3c.desc->ibi_cb = icm456xx_ibi_cb;

...
#ifndef ZEPHYR_DRIVERS_SENSOR_ICM456XX_H_
#define ZEPHYR_DRIVERS_SENSOR_ICM456XX_H_
...
typedef int (*icm456xx_bus_check_fn)(const union icm456xx_bus *bus);
typedef int (*icm456xx_reg_read_fn)(const union icm456xx_bus *bus, uint8_t reg, uint8_t *buf,
				    uint32_t size);
typedef int (*icm456xx_reg_write_fn)(const union icm456xx_bus *bus, uint8_t reg, uint8_t *buf,
				     uint32_t size);

struct icm456xx_bus_io {
	icm456xx_bus_check_fn check;
	icm456xx_reg_read_fn read;
	icm456xx_reg_write_fn write;
};

#if CONFIG_SPI
extern const struct icm456xx_bus_io icm456xx_bus_io_spi;
#endif

#if CONFIG_I2C
extern const struct icm456xx_bus_io icm456xx_bus_io_i2c;
#endif

struct icm456xx_data {
...
#endif
#ifdef CONFIG_ICM456XX_TRIGGER_OWN_THREAD
	K_KERNEL_STACK_MEMBER(thread_stack, CONFIG_ICM456XX_THREAD_STACK_SIZE);
	struct k_thread thread;
	struct k_sem gpio_sem;
#endif
#ifdef CONFIG_ICM456XX_TRIGGER_GLOBAL_THREAD
	struct k_work work;
#endif
};

struct icm456xx_config {
	union icm456xx_bus bus;
	const struct icm456xx_bus_io *bus_io;
	uint32_t serif_type;
...
#define ICM456XX_APEX_STATUS_MASK_TILT  BIT(0)
#define ICM456XX_APEX_STATUS_MASK_SMD   BIT(1)

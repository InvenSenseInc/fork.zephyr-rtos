...
static int icp201xx_read_reg_spi(const union icp201xx_bus *bus, uint8_t reg, uint8_t *rbuffer, uint32_t rlen)
{
	if (reg != 0) {
		int rc = 0;
...
static int icp201xx_write_reg_spi(const union icp201xx_bus *bus, uint8_t reg, const uint8_t *wbuffer, uint32_t wlen)
{
	if (reg != 0) {
		int rc = 0;

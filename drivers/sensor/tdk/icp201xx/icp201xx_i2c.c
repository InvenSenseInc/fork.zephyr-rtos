...
#if CONFIG_I2C
static int icp201xx_read_reg_i2c(const union icp201xx_bus *bus, uint8_t reg, uint8_t *rbuffer, uint32_t rlen)
{

	return i2c_burst_read_dt(&bus->i2c, reg, (uint8_t *)rbuffer, rlen);
}

static int icp201xx_write_reg_i2c(const union icp201xx_bus *bus, uint8_t reg, uint8_t *wbuffer, uint32_t wlen)
{

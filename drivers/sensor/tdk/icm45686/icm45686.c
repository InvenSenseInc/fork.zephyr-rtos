		...
		enum sensor_channel chan)
{
	...
	err = icm456xx_get_register_data(&data->driver, (inv_imu_sensor_data_t *)edata->payload.buf);
...
static int icm45686_init(const struct device *dev)
{
	...
	if (data->bus.rtio.type == ICM45686_BUS_SPI) {
		...
		err = icm456xx_write_reg(&data->driver, DRIVE_CONFIG0, 1, (uint8_t *)&drive_config0);
